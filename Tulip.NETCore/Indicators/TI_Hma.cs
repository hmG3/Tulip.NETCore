...
namespace Tulip
{
    ...
    {
        private static int HmaStart(double[] options)
        {
            int period = (int) options[0];
            int periodSqrt = (int) Math.Sqrt(period);
            return period + periodSqrt - 2;
        }

        private static int HmaStart(decimal[] options)
        {
            int period = (int) options[0];
            int periodSqrt = (int) Math.Sqrt(period);
            return period + periodSqrt - 2;
        ...
        private static int Hma(int size, double[][] inputs, double[] options, double[][] outputs)
        {
            ...
            var buff = BufferDoubleNew(periodSqrt);
        ...
        private static int Hma(int size, decimal[][] inputs, decimal[] options, decimal[][] outputs)
        {
            ...
            var buff = BufferDecimalNew(periodSqrt);
            int outputIndex = default;
            for (int i = period - 1; i < size; ++i)
            {
                weightSum += input[i] * period;
