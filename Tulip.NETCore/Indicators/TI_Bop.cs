...
namespace Tulip
{
    ...
    {
        private static int BopStart(double[] options)
        {
            return 0;
        }

        private static int BopStart(decimal[] options)
        {
            return 0;
        }
        ...
        private static int Bop(int size, double[][] inputs, double[] options, double[][] outputs)
        {
            ...
            for (var i = 0; i < size; ++i)
            {
                ...
                output[i] = Math.Max((close[i] - open[i]) / hl, 0.0);
            }
        ...
        private static int Bop(int size, decimal[][] inputs, decimal[] options, decimal[][] outputs)
        {
            ...
            for (var i = 0; i < size; ++i)
            {
                ...
                output[i] = Math.Max((close[i] - open[i]) / hl, Decimal.Zero);
            }
