...
{
    private static int LinRegStart(T[] options) => Int32.CreateTruncating(options[0]) - 1;

    private static int LinReg(int size, T[][] inputs, T[] options, T[][] outputs)
    {
        ...
        for (var i = period - 1; i < size; ++i)
        {
            ...
            T a = (y - b * x) * p;
            output[outputIndex++] = a + b * T.CreateChecked(period);
            xy -= y;
            y -= input[i - period + 1];
        }
