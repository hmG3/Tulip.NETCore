...
{
    private static int FoscStart(T[] options) => Int32.CreateTruncating(options[0]);

    private static int Fosc(int size, T[][] inputs, T[] options, T[][] outputs)
    {
        ...
        for (var i = period - 1; i < size; ++i)
        {
            ...
            if (i >= period)
            {
                output[outputIndex++] = THundred * (input[i] - tsf) / input[i];
            }

            tsf = a + b * T.CreateChecked(period + 1);
            xy -= y;
            y -= input[i - period + 1];
        }
